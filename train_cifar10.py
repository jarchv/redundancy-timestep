...
def main(args):
    ...
    # Data loading
    train_loader, train_size = utils.get_cifar10_data(args)
    ...
    for epoch in range(load_epoch + 1, args.epochs + 1):
        ...
        for image_batch, label_batch in train_loader:
            ...
            flops, macs, params = calculate_flops(model=model, args=[x[:1], y[:1]])
            print(f"FLOPs: {flops}, MACs: {macs}, Params: {params}")
            #flop_counter = FlopCounterMode(display=False)
            #loss = model(img=x, y=y)
...
#   Experiments
    ...
    parser.add_argument('--log_every', type=int, default=300, help='log after every "log_every" steps')
